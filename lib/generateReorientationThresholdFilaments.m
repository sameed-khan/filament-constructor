...
function reorient = generateReorientation(posPath, centerLine, orient, yBound, outputPath)
    outputPathFlag = true;
    if nargin < 5
    ...
    if ~orient
        utLine(:,2) = (utLine(:,2) - yBound) * -1;
        termPoints{:,'Pt Position Y'} = (termPoints{:,'Pt Position Y'} - yBound)*-1;
    end
