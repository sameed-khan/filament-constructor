...
function prominence = generateProminence(posPath, boundaryOne, boundaryTwo, centerLine, orient, yBound, outputPath)
    % Orient: +1 if cervix is at high Y-end, -1 if opposite
    % yBound: 1x2 matrix containing the minimum and maximum y-values of file
    if nargin < 7
        outputPath = false;
    end

    % Read in uterine centerline approximation
    utLine = readtable(centerLine, 'NumHeaderLines', 1, 'VariableNamingRule', 'preserve');
    utLine = table2array(utLine(:, {'Position X', 'Position Y', 'Position Z'}));
    utLine = sortrows(utLine,2);


    % Read in two boundaries
    bpts1 = readtable(boundaryOne, 'NumHeaderLines', 1, 'VariableNamingRule', 'preserve');
    bpts1 = table2array(bpts1(:, {'Position X', 'Position Y', 'Position Z'}));
    bpts1 = sortrows(bpts1,2);
    bpts1(1, 2) = 0;
    bpts1(end, 2) = yBound;

    bpts2 = readtable(boundaryTwo, 'NumHeaderLines', 1, 'VariableNamingRule', 'preserve');
    ...
    termPoints = readtable(posPath, 'NumHeaderLines', 1, 'VariableNamingRule', 'preserve');
